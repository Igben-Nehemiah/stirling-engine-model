...
class GasMixtures(WorkingFluid):
    ...
    def __init__(self, params: Parameters):
        ...
        self.e = pyenskog.Enskog(density, EOS, SonineOrder, verbose, 1, use_tables)
    ...
    def thermal_conductivity(self, temp):
        ...
        k1, k2 = self.params.fluid1.thermal_conductivity(temp), self.params.fluid2.thermal_conductivity(temp)
...
class StirlingEngine:
    ...
    def _adiabatic_analysis(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        print(f'Cooler Tk = {self.params.tk:.1f}[K], Heater Th = {self.params.th:.1f}[K]')
        ...
        while temp_error >= epsilon and iter_count < max_iterations:
            ...
            print(f'Iteration {iter_count}: Tc = {y[self.Index.TC]:.1f}[K], Te = {y[self.Index.TE]:.1f}[K]')
...
def main():
    ...
    params_mod = StirlingEngine.Parameters(
        freq=71.7,  # Operating frequency (Hz)
        charge_pressure=4.14e6,  # Charge pressure (Pa)
